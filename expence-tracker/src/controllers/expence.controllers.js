...
export const addNewExpence = AsyncHandler(async(req, res) => {
  ...
  const {title, type, description, amount, methodOfPayment } = req.body
  
  // ensure all data field required
  if([title, type, description, amount, methodOfPayment].some(field => (field === undefined))){
    return res
      .status(404)
  ...
  if([title, type, description, amount, methodOfPayment].some(field => field?.toString().trim() === "")){
    return res
    .status(404)
  ...
  const newExpence = {title, type, description, amount, methodOfPayment}
  let searchUser;
  try {
...
export const updateExpence = AsyncHandler(async(req, res) => {
  ...
  if (!req.body) {
    return res
      .status(404)
      .cookie("errorMessage", "DataError : Data not received", cookieExpire)
      .redirect(`/user/profile/updateExpence/${req.expenceId}`);
  }
  ...
  if(![title, type, description, amount, methodOfPayment].some(field => field)){
    return res
      .status(404)
      .cookie("errorMessage", "DataError : All field is required", cookieExpire)
      .redirect(`/user/profile/updateExpence/${req.expenceId}`);
  ...
  const updateExpence = {title, type, description, amount, methodOfPayment}
  try {
